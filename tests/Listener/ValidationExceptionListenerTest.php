...
class ValidationExceptionListenerTest extends AbstractTestCase
{
    ...
    public function testInvoke(): void
    {
        ...
        $event = $this->createEvent(new ValidationException(new ConstraintViolationList([
        ...
        $this->serializer->expects($this->once())
            ...
            ->with(
                $this->callback(function (ErrorResponse $response) {
                    ...
                    $violations = $details->getViolation();
                    if (1 !== count($violations) || 'validation failed' !== $response->getMessage()) {
                        return false;
                    }

                    return 'name' === $violations[0]->getFields() && 'error' === $violations[0]->getMessage();
                }),
                JsonEncoder::FORMAT
            )
